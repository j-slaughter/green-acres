// UI component to navigate between pages of invoices
'use client';
...
export default function Pagination({ totalPages }: { totalPages: number }) {
  ...
  // Get the current invoice page from search params
  const currentPage = Number(searchParams.get('page')) || 1;

  // createPageURL - generates the URL for the passed-in page number
  const createPageURL = (pageNumber: number | string) => {
    // Create URLSearchParams instance to manipulate URL query params
    const params = new URLSearchParams(searchParams);
    // Update search params with passed-in page number
    params.set('page', pageNumber.toString());
    return `${pathname}?${params.toString()}`;
  };

  // Generate the pagination navigation options (ex. [1,2,3,...,8])
